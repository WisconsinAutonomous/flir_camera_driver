...
: rclcpp::Node("flir_camera_synchronizer", options)
{
  ...
  const auto qos = rmw_qos_profile_sensor_data;//hopefully should resolve qos issue for synchronizer
...
void FlirCameraSynchronizer::syncTimeStamps(std::vector<ImageMsgPtr>& msgs) {  
  if (!msgs.empty()) {
    rclcpp::Time min_time = message_filters::message_traits::TimeStamp<ImageMsg>::value(*msgs[0]);
